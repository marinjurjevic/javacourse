...
public class Lexer {
	...
	/**
	 * string representing start of tag
	 */
	private final static String TAG_START = "{$";
	
	/**
	 * string representing end of tag
	 */
	...
	public Token nextToken() {
		...
		if (state == LexerState.TEXT) {
			...
			if (new String(data, currentIndex, 2).equals(TAG_START)) {
			...
			if (new String(data, currentIndex, 2).equals(TAG_END)) { // Check for
																	// tag end
	 ...
	 * integer or double.
	 * @param negative true if number is negative, false if number is positive
	...
	private Token generateString() {
		...
		while (data[currentIndex] != '\"') {			
			if (data[currentIndex] == '\\') { // possible escape
				currentIndex++;
				...
				if (data[currentIndex] == 'n'){
					...
					if (data[currentIndex] != '\"' 		// valid escapes
							&& data[currentIndex] != '\\') 
					{
